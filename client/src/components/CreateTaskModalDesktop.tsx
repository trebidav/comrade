      ...
      <div className="pip-panel" style={{ width: '480px', maxHeight: '85vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{
          ...
          <span style={{ fontSize: '0.75rem', letterSpacing: '0.1em', textTransform: 'uppercase' }}>
            {tab === 'task' ? 'Create Task' : 'Create Tutorial'}
          </span>
          <button
            onClick={onClose}
              ...
              <div style={{ marginBottom: '14px' }}>
                ...
                {photoPreview && (
                  <div style={{ position: 'relative', marginBottom: '8px' }}>
                    <img src={photoPreview} alt="Preview" style={{ width: '100%', maxHeight: '140px', objectFit: 'cover', borderRadius: '2px', border: '1px solid var(--pip-border)' }} />
                    <button
                      onClick={() => { setPhoto(null); setPhotoPreview(null) }}
                      style={{ position: 'absolute', top: '4px', right: '4px', background: 'rgba(0,0,0,0.6)', color: '#fff', border: 'none', borderRadius: '50%', width: '20px', height: '20px', cursor: 'pointer', fontSize: '12px', lineHeight: '20px', textAlign: 'center' }}
                    >×</button>
                  </div>
                )}
                {!photoPreview && (
                  <label style={{ display: 'block', padding: '10px', border: '1px dashed var(--pip-border)', textAlign: 'center', cursor: 'pointer', fontSize: '0.7rem', color: 'var(--pip-green-dark)' }}>
                    Click to add photo
